...
const ADMIN_CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .app-title span{color:var(--primary);font-size:1.4rem;font-weight:700;margin-left:0;}
...
const EMPTY_RESULT={date:'',opponent:'',score:'',competition:'Albi Mall Superliga',venue:'',matchday:''};
function ResultsTab({ token }) {
  ...
  function openEdit(m){setForm({date:m.date||'',opponent:m.opponent||'',score:m.score||'',competition:m.competition||'Albi Mall Superliga',venue:m.venue||'',matchday:m.matchday||''});setModal({mode:'edit',id:m.id});}
  ...
  return (
    <div>
      ...
      <div className="item-list">{matches.map(m=>(<div className="result-row" key={m.id}><span className="result-date">{m.date}</span><span className="result-info"><b>FC Malisheva</b> vs <b>{m.opponent}</b> <span className="result-score">{m.score}</span></span><span className="result-meta">{m.matchday?`${m.matchday} · `:''}{m.competition}</span><div style={{display:'flex',gap:'6px',flexShrink:0}}><button className="btn-sm" onClick={()=>openEdit(m)}>{tr('edit')}</button><button className="btn-sm btn-danger" onClick={()=>del(m.id)}>{tr('del')}</button></div></div>))}</div>
